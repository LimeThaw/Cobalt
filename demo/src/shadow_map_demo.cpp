...
int main() {
    ...
    material::texture_bindings shadow_map_material_bindings;
    for(int i = 0; i < 4; ++i) {
        shadow_maps.push_back(std::make_shared<texture2d>(512, 512, GL_DEPTH_COMPONENT32, GL_DEPTH_COMPONENT,
                              GL_CLAMP, GL_CLAMP, GL_NEAREST, GL_NEAREST));
	shadow_map_material_bindings.push_back(
	  material::texture_binding("shadow_map[" + std::to_string(i) + "]", shadow_maps.back()));
    }
    ...
    shadow_map_material_bindings.push_back(material::texture_binding("color_map", map_tex));
    auto map_mat = std::make_shared<material>(shadow_map_material_bindings);

    auto rockwall_tex = std::make_shared<texture2d>(
                            texture_cache::get_instance().get_texture_from_filename(texture_dir + "rockwall.jpg"));
    
    shadow_map_material_bindings[4] = material::texture_binding("color_map", rockwall_tex);
    auto room_mat = std::make_shared<material>(shadow_map_material_bindings);
    ...
    for(int i = 0; i < 5; ++i) {
        ...
        room_node->place(-10 + 20.0f * (std::rand() / (float) RAND_MAX), 10.0f * (std::rand() / (float) RAND_MAX), -10 + 20.0f * (std::rand() / (float) RAND_MAX));
    ...
    glm::vec3 ambient_light_color = glm::vec3(0);
    ...
    std::vector<glm::mat4> shadow_map_view_projection_matrices;
    std::vector<std::shared_ptr<uniform>> shadow_map_view_projection_matrices_uniform;
    ...
    std::vector<glm::vec3> light_colors { glm::vec3(0.3f), glm::vec3(0.3f), glm::vec3(0.3f), glm::vec3(0.1f) };
    std::vector<glm::vec3> light_directions { glm::vec3(0.71f, 0.71f, 0), glm::vec3(-0.38f, 0.71f, 0.64f), glm::vec3(0.38f, 0.71f, 0.64f),
      glm::vec3(0, 1, 0) };
    
    for(int i = 0; i < 4; ++i) {
      auto shadow_map_attachment = std::make_shared<texture_framebuffer_attachment>(shadow_maps[i]);
      ...
      auto direction = light_directions[i];
      directional_lights.push_back(directional_light(
	light_colors[i], 1, direction));
      
      
      auto shadow_map_view_matrix = glm::lookAt(direction * 100.0f, glm::vec3(0, 0, 0), glm::vec3(0, 0, 1));
      auto shadow_map_projection_matrix = glm::ortho(-100.0f, 100.0f, -100.0f, 100.0f, 0.1f, 1000.0f);
      auto shadow_map_view_projection_matrix = shadow_map_projection_matrix * shadow_map_view_matrix;
      
      shadow_map_view_projection_matrices.push_back(shadow_map_view_projection_matrix);
      
      //also apply bias, because tex coord is [0;1], clip space is [-1;1]
      shadow_map_view_projection_matrices_uniform.push_back(
	std::make_shared<mat4_uniform>(bias_matrix * shadow_map_view_projection_matrix));
      
   }
   
    map_mat->set_uniform("shadow_map_view_projection_matrix",
			 std::make_shared<array_uniform>(shadow_map_view_projection_matrices_uniform));
    room_mat->set_uniform("shadow_map_view_projection_matrix",
			 std::make_shared<array_uniform>(shadow_map_view_projection_matrices_uniform));
	...
	for(int i = 0; i < 4; ++i) {
	  ...
	  shadow_pass.render(my_world, shadow_map_view_projection_matrices[i], *shadow_map_framebuffers[i]);
	}
