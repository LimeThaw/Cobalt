...
class pointer_wrapper : public shared_ptr<T> {
	public:
		...
		template<class S>
		pointer_wrapper(const pointer_wrapper<S> &other) : shared_ptr<T>(dynamic_pointer_cast<T>(other)) {}
		/** Same as the other, just without constant modifyer. */
		 ...
		 */
		template<class... Args>
		explicit pointer_wrapper(const Args&... args) : shared_ptr<T>() {
			make(args...);
		}
