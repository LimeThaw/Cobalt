...
class texture_link {
    public:
        texture_link(const char* texture_path, const char* uniform_name);///< Load a new texture and create a texture_link for it.
        texture_link(texture_id new_texture, const char* uniform_name);///< Create a texture_link for a texture that is already loaded.
        ~texture_link();
        void set_texture(const char* path);///< Load a new texture and link it to the uniform.
        void set_texture(texture_id new_texture);///< Link a texture to the uniform that has already been loaded.
        void set_uniform(const char* uniform_name);///< Defines a new uniform name to link the texture to.
