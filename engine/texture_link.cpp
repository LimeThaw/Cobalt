...
texture_link::texture_link(const std::string &texture_path, const std::string &uniform_name) {
    set_texture(texture_path);
    set_uniform(uniform_name);
}

texture_link::texture_link(unsigned int new_texture, const std::string &uniform_name) {
...
void texture_link::set_texture(const std::string &path) {
...
void texture_link::set_uniform(const std::string &uniform_name) {
...
void texture_link::apply(unsigned int place) {
    if(place <= GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS) {
        ...
        glUniform1i(glGetUniformLocation(shader_id, uniform_location.c_str()), place);
    }else {
